...
print(
    f"- Number of unique session IDs: {df_answers['sessionId'].unique().shape[0]:,}")
...
print(f" - Demo: {df_individuals[(df_individuals['informationType'] == 'demographics')
...
print()
print("For all answers with the same session ID, keep only the last answer using createdAt")
...
print(
    f"Number of unique session IDs from answers: {df_answers_last.shape[0]:,}")
...
df_individuals["startAt"] = df_individuals["createdAt"] - \
    pd.to_timedelta(df_individuals["secondsElapsed"], unit="s")
...
df_individuals_rme = df_individuals[df_individuals["informationType"] == "rmeTen"].copy(
    deep=True)
df_individuals_demo = df_individuals[(df_individuals["informationType"] == "demographics")
                                     | (df_individuals["informationType"] == "demographicsLongInternational")].copy(deep=True)
...
for df in [df_individuals_crt, df_individuals_rme, df_individuals_demo]:
    ...
    df.drop_duplicates(
        subset=["userSessionId"], keep="last", inplace=True
    )
...
def find_closest_triplets(
    ...
    triplet_match_results = pd.DataFrame(index=df_individuals_demo_rem.index,
                                         columns=["num-crt-matches", "crt", "diff-crt",
                                                  "num-rme-matches", "rme", "diff-rme"])
    triplet_match_results.index.name = "demo"

    for demo_id in tqdm(triplet_match_results.index, ):
        ...
        diff_demo_crt = (demo_start_time -
                         df_individuals_crt_rem["startAt"]).abs()
        diff_demo_threshold = diff_demo_crt[diff_demo_crt <= pd.Timedelta(
            seconds=threshold)]
        diff_demo_rme = (demo_start_time -
                         df_individuals_rme_rem["startAt"]).abs()
        diff_demo_rme_threshold = diff_demo_rme[diff_demo_rme <= pd.Timedelta(
            seconds=threshold)]

        triplet_match_results.at[demo_id,
                                 "num-crt-matches"] = diff_demo_threshold.shape[0]
        triplet_match_results.at[demo_id,
                                 "num-rme-matches"] = diff_demo_rme_threshold.shape[0]

        triplet_match_results.at[demo_id, "crt"] = diff_demo_crt.idxmin()
        triplet_match_results.at[demo_id,
                                 "diff-crt"] = diff_demo_crt.min().total_seconds()
        triplet_match_results.at[demo_id, "rme"] = diff_demo_rme.idxmin()
        triplet_match_results.at[demo_id,
                                 "diff-rme"] = diff_demo_rme.min().total_seconds()
    ...
    triplet_only_one_dedup = triplet_only_one \
        .sort_values(by="diff-crt").groupby("crt").nth(0) \
        .sort_values(by="diff-rme").groupby("rme").nth(0)
...
for i in range(1, 100):
    ...
    df_individuals_demo_rem = df_individuals_demo.drop(
        index=processed_demo_ids)
    triplet_results = find_closest_triplets(
        df_individuals_crt_rem, df_individuals_rme_rem, df_individuals_demo_rem, threshold=1.5)

    triplet_results["iter"] = i
    triplet_results = triplet_results[[
        "crt", "rme", "demo", "iter", "diff-crt", "num-crt-matches", "diff-rme", "num-rme-matches"]]
    ...
    if triplet_results_all is None:
        ...
        triplet_results_all = pd.concat(
            [triplet_results_all, triplet_results], axis=0).reset_index(drop=True)
...
print()
print("Number of triplets for which there is exactly one match for CRT and RME:"
...
triplet_results_all["crt_created"] = df_individuals_crt.loc[triplet_results_all["crt"], "createdAt"].values
triplet_results_all["crt_started"] = df_individuals_crt.loc[triplet_results_all["crt"], "startAt"].values

triplet_results_all["rme_created"] = df_individuals_rme.loc[triplet_results_all["rme"], "createdAt"].values
triplet_results_all["rme_started"] = df_individuals_rme.loc[triplet_results_all["rme"], "startAt"].values

triplet_results_all["demo_created"] = df_individuals_demo.loc[triplet_results_all["demo"], "createdAt"].values
triplet_results_all["demo_started"] = df_individuals_demo.loc[triplet_results_all["demo"], "startAt"].values
...
for round_number in range(1, 101):
    ...
    if triplets_plus_answers is None:
        ...
        remaining_triplets = triplet_results_all[~triplet_results_all["crt"].isin(
            triplets_plus_answers["crt"])].copy()

    print(f"Round {round_number}: {remaining_triplets.shape[0]:,} remaining triplets")

    num_matches_this_round = 0

    for i, row in tqdm(remaining_triplets.iterrows(), total=remaining_triplets.shape[0]):
        ...
        time_diff_crt = (crt_start_time -
                         df_answers_last_rem["createdAt"]).abs()
        time_diff_rme = (rme_start_time -
                         df_answers_last_rem["createdAt"]).abs()
        time_diff_demo = (demo_start_time -
                          df_answers_last_rem["createdAt"]).abs()

        # Element-wise min
        time_diff = pd.concat(
            [time_diff_crt, time_diff_rme, time_diff_demo], axis=1).min(axis=1)
        ...
        if time_diff.shape[0] > 0:
            ...
            remaining_triplets.at[i,
                                  "diff-answers"] = time_diff.min().total_seconds()
    ...
    remaining_triplets = remaining_triplets.sort_values(
        by="diff-answers").groupby("answers").nth(0).reset_index(drop=False)

    print(" - Number of matches after deduplication:",
          remaining_triplets.shape[0])
    ...
    if triplets_plus_answers is None:
        ...
        triplets_plus_answers = pd.concat(
            [triplets_plus_answers, remaining_triplets], axis=0).reset_index(drop=True)
...
print()
print(f"Total number of complete (answer-CRT-RME-Demo) matches: {triplets_plus_answers.shape[0]:,}")
...
for session_id in tqdm(common_session_ids, desc="Processing common sessions",
                       unit=" sessions", leave=True):
