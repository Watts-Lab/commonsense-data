...
function sgRenderDetailList(cacheKey, metric) {
  ...
  body.innerHTML = statements.map((entry, index) => {
    ...
    const totalTried = entry.generations.reduce((sum, g) => sum + g.n_judges_tried, 0);
    ...
    return `<tr data-index="${index}">` +
      ...
      `<td class="sg-col-num-judges"><span class="sg-judge-count">${totalJudges} / ${totalTried}</span></td>` +
      '</tr>';
  }).join('');
}
...
function sgRenderGeneration(gen, row, multi) {
  const heading = multi
    ? `<h3 class="sg-generation-heading">Generated by ${esc(gen.gen_model)} ` +
      `<span class="sg-judge-count">${gen.judges.length} / ${gen.n_judges_tried}</span></h3>`
    : '';
  ...
  const featureSections = SG_FEAT_DEFS.map(def => {
    ...
    const judgePanels = gen.judges.map(judge => sgAssessmentPanel(`Judge: ${judge.judge_model}`, {
...
function sgShowStatement(entry) {
  ...
  const totalTried = generations.reduce((sum, g) => sum + g.n_judges_tried, 0);
  ...
  document.getElementById('sgStatementModel').textContent =
    `${genSummary} · ${totalJudges} / ${totalTried} judges confirmed common sense with full feature agreement`;
