...
function sgRenderGeneration(gen, row, multi) {
  ...
  const commonsenseSection = `<section class="sg-statement-feature"><h3>Human common sense</h3>` +
  ...
  const featureSections = SG_FEAT_DEFS.map(def => {
    ...
    return `<section class="sg-statement-feature"><h3>${esc(label)} <span class="${badgeClass}">${esc(badgeText)}</span></h3>` +
